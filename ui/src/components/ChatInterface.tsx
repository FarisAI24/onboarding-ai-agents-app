...
export default function ChatInterface({ userId, userName, onTaskUpdate }: ChatInterfaceProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white/60 dark:bg-surface-800/60 backdrop-blur-lg rounded-2xl shadow-xl border border-surface-200/50 dark:border-surface-700/50">
      {/* Header */}
      <div className="flex items-center gap-3 p-4 border-b border-surface-200/50 dark:border-surface-700/50">
        ...
        <div>
          <h2 className="font-semibold text-surface-900 dark:text-surface-100">Onboarding Copilot</h2>
          <p className="text-xs text-surface-500">AI-powered assistant • Always here to help</p>
        </div>
      </div>
      ...
      {messages.length <= 2 && faqTopics.length > 0 && (
        <div className="p-4 border-b border-surface-200/50 dark:border-surface-700/50">
          <p className="text-sm text-surface-600 dark:text-surface-400 mb-3 flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {faqTopics.slice(0, 6).map((topic) => (
              <button
                ...
                className="px-3 py-1.5 text-sm bg-surface-100 dark:bg-surface-700 hover:bg-primary-100 dark:hover:bg-primary-900/30 text-surface-700 dark:text-surface-300 rounded-full transition-colors border border-surface-200 dark:border-surface-600 hover:border-primary-300 dark:hover:border-primary-700"
              >
                {topic.title}
              </button>
            ))}
        ...
        <AnimatePresence>
          {messages.map((message, index) => (
            ...
            >
              ...
              >
                {message.role === 'assistant' ? (
                  <div className="prose prose-sm dark:prose-invert max-w-none prose-p:my-1 prose-ul:my-1 prose-ol:my-1 prose-li:my-0.5 prose-headings:my-2 prose-headings:text-surface-900 dark:prose-headings:text-surface-100">
                ...
                {message.sources && message.sources.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-surface-200/30 dark:border-surface-600/30">
                    <p className="text-xs text-surface-500 dark:text-surface-400 mb-2 flex items-center gap-1">
      ...
      <div className="p-4 border-t border-surface-200/50 dark:border-surface-700/50">
        <div className="flex items-center gap-2 bg-surface-100 dark:bg-surface-700 rounded-xl px-4 py-2">
          <input
            ref={inputRef}
            type="text"
            value={input}
            ...
            className="flex-1 bg-transparent border-none outline-none text-surface-800 dark:text-surface-200 placeholder:text-surface-400"
            disabled={isLoading}
          />
          <button
            onClick={() => handleSend()}
